//chatgptden komek almisam biraz
...
let goodsContainer = document.getElementById("goods-container");
...
function createGoods() {
    goods.forEach((good) => {
        ...
        box.innerHTML = `
            ...
            <p>${good.product_description}</p>
            <p>Price: ${good.product_price} AZN</p>
            <p>Store: ${good.store_name}</p>
            <p>Address: ${good.store_address}</p>
            <button onclick="saveGood('${good.id}')">Save</button>
        `;
        goodsContainer.appendChild(box);
    });
}

function saveGood(id) {
    let selectedGood = goods.find((good) => good.id === id);
    let savedGoods = JSON.parse(localStorage.getItem("savedGoods")) || [];
    if (!savedGoods.some((good) => good.id === id)) {
        savedGoods.push(selectedGood);
        localStorage.setItem("savedGoods", JSON.stringify(savedGoods));
        alert(`${selectedGood.product_name} added to your saved goods.`);
    } else {
        alert(`${selectedGood.product_name} is already in your saved goods.`);
    }
}
